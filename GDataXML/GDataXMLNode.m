...
static const int kGDataXMLParseOptions = (XML_PARSE_NOCDATA |  XML_PARSE_DTDATTR);
...
{
	NSMutableString* mutableXmlString = [[NSMutableString alloc] initWithString:[element stringValue]];
	
	[mutableXmlString replaceOccurrencesOfString:string withString:@"" options:NSCaseInsensitiveSearch 
										   range:NSMakeRange(0, [mutableXmlString length])];
	[element setStringValue:mutableXmlString];
	[mutableXmlString release];
